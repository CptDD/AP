...
computeVFH (const boost::filesystem::path &path)
{
  ...
  std::cout<<"Computing the vfh!"<<std::endl;
...
bool
loadHist (const boost::filesystem::path &path, vfh_model &vfh, bool get_fpfh)
{


  std::cout<<"Computing the histograms!"<<std::endl;
  int vfh_idx;
...
loadFeatureModels (const boost::filesystem::path &base_dir, const std::string &extension, 
                   std::vector<vfh_model> &models, bool get_fpfh)
{

  std::cout<<"Loading the feature models!"<<std::endl;
  ...
  for (boost::filesystem::directory_iterator it (base_dir); it != boost::filesystem::directory_iterator (); ++it)
  {
    if (boost::filesystem::is_directory (it->status ()))
    {
      ...
      loadFeatureModels (it->path (), extension, models, get_fpfh);
    }
    if (boost::filesystem::is_regular_file (it->status ()) && boost::filesystem::extension (it->path ()) == extension)
    {
      vfh_model m;
      if (loadHist (base_dir / it->path ().filename (), m, get_fpfh))
        models.push_back (m);
    }
  }
}
...
main (int argc, char** argv)
{
  ...
  loadFeatureModels (argv[1], extension, models, true);
  loadFeatureModels (argv[1], extension, models, false);
